...
// Estimator fuses 3-axis accelerometer, gyroscope and magnetometer data into
// an attitude estimate using a nonlinear Passive Complementary Filter.
//
...
//
// 	e := attestimator.Estimator{}
//
// 	// Initialise the estimator (none of these are actually strictly required for the estimator to work, and can be set at any time)
// 	e.SetMagCalib(0.68, -1.32, 0.0);         // Recommended: Use if you want absolute yaw information as opposed to just relative yaw (Default: (1, 0, 0))
// 	e.SetPIGains(2.2, 2.65, 10, 1.25);       // Recommended: Use if the default gains (shown) do not provide optimal estimator performance (Note: Ki = Kp/Ti)
// 	e.SetQLTime(2.5);                        // Optional: Use if the default quick learning time is too fast or too slow for your application (Default: 3)
// 	e.SetAttitude(0.5, 0.5, 0.5, 0.5);       // Optional: Use if you have prior knowledge about the orientation of the robot (Default: Identity orientation)
// 	e.SetAttitudeEuler(M_PI, 0.0, 0.0);      // Optional: Use if you have prior knowledge about the orientation of the robot (Default: Identity orientation)
// 	e.SetAttitudeFused(M_PI, 0.0, 0.0, 1.0); // Optional: Use if you have prior knowledge about the orientation of the robot (Default: Identity orientation)
// 	e.SetGyroBias(0.152, 0.041, -0.079);     // Optional: Use if you have prior knowledge about the gyroscope bias (Default: (0, 0, 0))
// 	e.SetAccMethod(Est.ME_FUSED_YAW);        // Optional: Use if you wish to experiment with varying acc-only resolution methods
//
// 	// Main loop
// 	while(true) {
// 		...
...
// 		e.Update(0.020, g[0], g[1], g[2], a[0], a[1], a[2], m[0], m[1], m[2]);
// 		...
// 		double q[4];
// 		e.GetAttitude(q);
...
func (e *Estimator) Update(dt, gyroX, gyroY, gyroZ, accX, accY, accZ, magX, magY, magZ float64) {
...
func (e *Estimator) updateQy(accX, accY, accZ, magX, magY, magZ float64) {
